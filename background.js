  ...
  try {
      ...
      func: async () => {
        ...
        try {
          ...
          const cd = await api(
            ...
            "&sysparm_fields=sys_id,number,short_description,priority,u_name_first_comment" +
    ...
    if (!seenIds.size) {
      await chrome.storage.local.set({
        ...
        fr_last_result: { initialized: true, notified: [] }
      });
      console.log("[FirstResponse] Estado inicial salvo. Notificações serão enviadas para novos casos a partir da próxima verificação.");
      return;
    }
    ...
    if (newlyArrived.length) {
      for (const c of newlyArrived) {
        ...
        notify(
          ...
          `🆕 Novo chamado na fila: ${c.number}`,
          `Prioridade: ${prio} | ${desc}`
        );
      }
    }
